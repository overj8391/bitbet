                ...
                try
                {
                    ...
                    switch( $aid ) 
                    {
                        ...
                        case 'settings':
                            ...
                            $result_tmp[] = '{"complex":{"balance":' . $balanceInCents . ',"balanceRaw":' . round($balanceInCents * $slotSettings->CurrentDenom) . ',"denominations":[' . implode(',', $denoms) . '],"bets":[[[' . $gameBets[0] . ',1,7000],[' . $gameBets[1] . ',1,14000],[' . $gameBets[2] . ',1,21000],[' . $gameBets[3] . ',1,42000],[' . $gameBets[4] . ',2,70000]],[[1,1,14000],[2,1,28000],[3,1,42000],[4,1,56000],[5,2,70000]],[[1,1,14000],[2,1,28000],[3,1,42000],[4,1,56000],[5,2,70000]],[[1,1,14000],[2,1,28000],[3,1,42000],[4,1,56000],[5,2,70000]],[[1,1,14000],[2,1,28000],[3,1,42000],[4,1,56000],[5,2,70000]]],"paytables":{"1":[1,2,3,4,5,9,25,50,250],"2":[1,2,3,4,5,9,25,50,800]},"jackpot":true,"jackpotMinBet":1,"jackpotMaxBet":100000},"gameIdentificationNumber":' . $postData['gameIdentificationNumber'] . ',"gameNumber":-1,"sessionKey":"f470855a38998b4c1678a0fd7d0ea815","msg":"success","messageId":"' . $postData['messageId'] . '","qName":"app.services.messages.response.GameResponse","command":"settings","eventTimestamp":1567091239811}';
                            break;
                        case 'subscribe':
                            $hist = [
                                78, 
                        ...
                        case 'ping':
                            $cashBackBtn = 0;
                            $refund = \VanguardLTE\Refund::where('shop_id', Auth::user()->shop_id)->first();
                            if( $refund && auth()->user()->present()->count_refund > 0 && auth()->user()->present()->balance <= $refund->min_balance ) 
                            {
                                $cashBackBtn = 1;
                            }
